...
if [[ $MODE == 'train' ]]; then
  if [[ $TRAIN_MODALITY == 'all' ]]; then
    ...
    for CONTRAST in "${MODALITIES[@]}"; do
      ...
      START_TIME=$(date +%s)
      # Using $TRAIN here for consistency and to pass resume arguments if provided
      python scripts/train.py $COMMON $TRAIN --contr=${CONTR}
      END_TIME=$(date +%s)
    ...
    START_TIME=$(date +%s)
    # Corrected: Include $TRAIN to pass resume_checkpoint and resume_step
    python scripts/train.py $COMMON $TRAIN --contr=${CONTR}
    END_TIME=$(date +%s)
    ELAPSED=$((END_TIME - START_TIME))
